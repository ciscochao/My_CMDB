...
class Asset(models.Model):
    ...
    asset_type = models.CharField(choices=asset_type_choice, max_length=64, default='other', verbose_name="资产类型")
    ...
    manufacturer = models.ForeignKey('Manufacturer', null=True, blank=True, verbose_name="厂商",
                                     on_delete=models.SET_NULL)
    manage_ip = models.GenericIPAddressField(null=True, blank=True, verbose_name="管理IP地址")
    asset_tags = models.ManyToManyField('Tag', blank=True, verbose_name="资产标签")
    asset_admin = models.ForeignKey(User, null=True, blank=True, related_name='asset_admin', on_delete=models.SET_NULL,
                                    verbose_name="资产管理员")
...
class Server(models.Model):
    ...
    # 一对一关联，asset被删除时一起删除server
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, default=2, verbose_name="服务器类型")
    create_by = models.CharField(choices=create_by_choice, max_length=32, default='manual', verbose_name="添加方式")
    hosted_on = models.ForeignKey('self', related_name='hosted_on_server', blank=True, null=True,
                                  on_delete=models.CASCADE, verbose_name="宿主机")
...
class NetworkDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, default=1, verbose_name="网络设备类型")
    ...
    port_num = models.SmallIntegerField(null=True, blank=True, verbose_name="端口数量")
    device_detail = models.TextField(null=True, blank=True, verbose_name="设备详细配置")
...
class Interface(models.Model):
    ...
    base_on = models.ForeignKey('NetworkDevice', on_delete=models.CASCADE)
    name = models.CharField('接口名称', max_length=64, unique=True)
    vlan = models.IntegerField('vlan信息', default=1)
    vlan_name = models.CharField('vlan名称', max_length=64)
    state = models.SmallIntegerField(choices=sub_asset_type_choice, default=1)

    def __str__(self):
        return '%s is %s' % (self.name, self.state)
...
class SoftWare(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, default=5, verbose_name="=软件类型")
    license_num = models.IntegerField(default=1, verbose_name="授权数量")
    version = models.CharField(max_length=128, unique=True, help_text="例如：Cent 5.2.9-2kali1", verbose_name="相关版本")
...
class BusinessUnit(models.Model):
    ...
    memo = models.TextField('备注', max_length=64, blank=True, null=True)
...
class Contract(models.Model):
    ...
    license_num = models.IntegerField('授权数量', blank=True, null=True)
